...
class DFA extends database_1.DBUnit {
    ...
    createDispatchers() {
        const funs = {
            "dispatch_effect": (fname, eid, closure) => this.db.inner.run(`
                SELECT CASE ${fname}
                ...
                END`),
            "dispatch_condition": (fname, eid, closure) => this.db.inner.run(`
                SELECT CASE ${fname}
    ...
    createConditionUDFs() {
        const funs = {
            ...
            "cond_is_at": (eid, x, y) => this.db.getSingleValue(`
                            SELECT 
                                (${x},${y}) = a.position 
                            FROM 
    ...
    createEffectUDFs() {
        const funs = {
            ...
            "eff_pathsearch_simple": (eid) => this.db.inner.run(``)
        };
        this.createUDFs(funs);
    }
}
