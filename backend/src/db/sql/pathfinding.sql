...
RETURNS VOID AS $$
    SELECT pathfinding.init_search(
        _eid1, 
        (SELECT POINT(x,y) FROM environment.position_components WHERE entity_id = _eid1),
        (SELECT POINT(x,y) FROM environment.position_components WHERE entity_id = _eid2)
    );
$$ LANGUAGE sql;--
